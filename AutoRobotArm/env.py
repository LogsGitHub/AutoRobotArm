...
class ArmEnv(object):
    ...
    def step(self, action):
        ...
        r = -np.sqrt(dist2[0] ** 2 + dist2[1] ** 2) - np.abs(self.arm_info['r'][1]) * 0.1
        # done and reward
...
'''
# 这是env.py部分：定义一个ArmEnv类，用来模拟一个双臂机器人的环境
class ArmEnv(object):
    ...
    def step(self, action): # 定义一个步进方法，用来更新状态和奖励
        ...
        dist2 = [(self.goal['x'] - finger[0]) / 400, (self.goal['y'] - finger[1]) / 400] # 计算第二个臂的终点到目标中心的距离，并除以400进行归一化
        r = -np.sqrt(dist2[0]**2+dist2[1]**2) # 根据第二个臂的终点到目标中心的距离计算奖励值，距离越小奖励越大

        # 判断是否结束和奖励
        if self.goal['x'] - self.goal['l']/2 < finger[0] < self.goal['x'] + self.goal['l']/2: # 如果第二个臂的终点的x坐标在目标的范围内
            if self.goal['y'] - self.goal['l']/2 < finger[1] < self.goal['y'] + self.goal['l']/2: # 如果第二个臂的终点的y坐标在目标的范围内
                r += 1. # 奖励值加1
                self.on_goal += 1 # 记录达到目标的次数加1
                if self.on_goal > 50: # 如果达到目标的次数超过50次
                    done = True # 结束为真
        else: # 否则
            self.on_goal = 0 # 记录达到目标的次数归零

        # 状态
        s = np.concatenate((a1xy_/200, finger/200, dist1 + dist2, [1. if self.on_goal else 0.])) # 将所有特征拼接成一个数组，作为状态
...
class Viewer(pyglet.window.Window):
    ...
    def __init__(self, arm_info, goal): # 初始化方法，传入两个臂的信息和目标信息
        # vsync=False to not use the monitor FPS, we can speed up training
        super(Viewer, self).__init__(width=400, height=400, resizable=False, caption='Arm', vsync=False) # 调用父类的初始化方法，设置窗口的宽度，高度，是否可调整大小，标题和垂直同步
