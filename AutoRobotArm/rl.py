...
'''
# 导入torch，torch.nn，torch.optim和numpy模块
import torch
import torch.nn as nn
import torch.optim as optim
import numpy as np
...
class DDPG(object): # 定义一个DDPG类，用来实现DDPG算法
    def __init__(self, a_dim, s_dim, a_bound,): # 初始化方法，传入动作维度，状态维度和动作范围
        self.memory = np.zeros((MEMORY_CAPACITY, s_dim * 2 + a_dim + 1), dtype=np.float32) # 定义一个数组，用来存储记忆库中的转移元组（s,a,r,s_）
    ...
    def learn(self): # 定义一个学习的方法，用来更新网络参数
        # soft target replacement
        for target_param, eval_param in zip(self.actor_target.parameters(), self.actor_eval.parameters()): # 遍历actor目标网络和评估网络的参数
            target_param.data.copy_(TAU * eval_param.data + (1 - TAU) * target_param.data) # 用软更新的方式更新目标网络的参数

        for target_param, eval_param in zip(self.critic_target.parameters(), self.critic_eval.parameters()): # 遍历critic目标网络和评估网络的参数
            target_param.data.copy_(TAU * eval_param.data + (1 - TAU) * target_param.data) # 用软更新的方式更新目标网络的参数
