...
class ArmEnv(object):
    ...
    def __init__(self):
        self.arm_info = np.zeros(2, dtype=[
            ('l', np.float32),
            ('r', np.float32),
            ('joints', np.ndarray, (2,))
        ])
        ...
        self.num_joints = 2
        self.arm_info['joints'] = np.array([(0, 0)] * self.num_joints)
        self.obstacle = {'x': 200., 'y': 70., 'l': 40, 'color': (0, 100, 0)}

    def _update_joints(self):
        # 计算关节位置
        center_x, center_y = 200, 200  # 将中心点设为 (200, 200)
        self.arm_info['joints'] = np.array([(
            center_x + np.sum(self.arm_info['l'][0:i]) * np.cos(np.sum(self.arm_info['r'][0:i + 1])),
            center_y + np.sum(self.arm_info['l'][0:i]) * np.sin(np.sum(self.arm_info['r'][0:i + 1])))
            for i in range(self.num_joints)])

    def _is_collision(self, finger):
        # 计算障碍物的边界
        obstacle_left = self.obstacle['x'] - self.obstacle['l'] / 2
        obstacle_right = self.obstacle['x'] + self.obstacle['l'] / 2
        obstacle_top = self.obstacle['y'] - self.obstacle['l'] / 2
        obstacle_bottom = self.obstacle['y'] + self.obstacle['l'] / 2


        # 创建障碍物矩形
        obstacle_rect = geom.Polygon([(obstacle_left, obstacle_top),
                                      (obstacle_right, obstacle_top),
                                      (obstacle_right, obstacle_bottom),
                                      (obstacle_left, obstacle_bottom)])

        # 检查从基座（坐标原点）到第一个关节的机械臂部分是否与障碍物边界相交
        joint0 = (200, 200)
        joint1 = self.arm_info['joints'][1]
        arm_segment1 = geom.LineString([joint0, joint1])


        if obstacle_rect.intersects(arm_segment1):
            return True

        # 检查从第一个关节到机械臂末端的线段是否与障碍物相交
        arm_segment2 = geom.LineString([joint1, finger])

        if obstacle_rect.intersects(arm_segment2):
            print(22222)
            return True
    ...
    def step(self, action):  # 定义一个步进方法，用来更新状态和奖励
        ...
        # r += action_cost
        is_collision = self._is_collision(finger)
        # print(self.arm_info['joints'][1])
        # print(self.arm_info['joints'])
        if is_collision:
            print(2)
            r = -100  # 或者其他负奖励值
            done = True  # 结束当前试验
    ...
    def render(self):  # 定义一个渲染方法，用来显示环境
        if self.viewer is None:  # 如果可视化对象为空
            self.viewer = Viewer(self.arm_info, self.goal, self.obstacle)  # 创建一个Viewer对象，传入两个臂的信息和目标信息
...
class Viewer(pyglet.window.Window):
    ...
    def __init__(self, arm_info, goal, obstacle_info):  # 初始化方法，传入两个臂的信息和目标信息
        ...
        self.obstacle = self.batch.add(
            4, pyglet.gl.GL_QUADS, None,
            ('v2f', (self.obstacle_info['x'] - self.obstacle_info['l'] / 2,
                     self.obstacle_info['y'] - self.obstacle_info['l'] / 2,
                     self.obstacle_info['x'] + self.obstacle_info['l'] / 2,
                     self.obstacle_info['y'] - self.obstacle_info['l'] / 2,
                     self.obstacle_info['x'] + self.obstacle_info['l'] / 2,
                     self.obstacle_info['y'] + self.obstacle_info['l'] / 2,
                     self.obstacle_info['x'] - self.obstacle_info['l'] / 2,
                     self.obstacle_info['y'] + self.obstacle_info['l'] / 2)),
            ('c3B', self.obstacle_info['color'] * 4)  # 深绿色
        )
